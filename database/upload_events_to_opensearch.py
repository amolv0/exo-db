...
region = 'us-east-1'
service = 'es'
dynamodb_table_name = 'event-data'  # Updated table name
index_name = 'team-data'  # Updated index name, change as needed
opensearch_endpoint = 'https://search-team-data-search-xaeptdqqk2djjjmer2bq63eetq.us-east-1.es.amazonaws.com'  # Update with your endpoint
...
def extract_relevant_attributes(item):
    # Updated to extract only 'id', 'name', and 'region'
    return {
...
def create_index_with_mapping():
    # Updated mapping for 'id', 'name', and 'region'
    mapping = {
        "mappings": {
            "properties": {
                "id": {"type": "keyword", "index": False},  # Assuming 'id' as a unique identifier
                "name": {"type": "text"},  # 'name' as searchable text
